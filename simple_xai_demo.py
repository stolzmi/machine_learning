"""
Simple XAI demonstration with random weights
This shows how the XAI methods work without needing a trained model
"""
...
from cifar10_cnn_model import CIFAR10CNN, initialize_model
...
def load_sample_image():
    ...
    ds = tfds.load('cifar10', split='test', as_supervised=False)
...
def main():
    print("="*60)
    print("Simple XAI Demo - CIFAR-10")
    ...
    print(f"Predicted: {CIFAR10_CLASSES[pred_class]} ({float(probs[pred_class]):.2%})")
    ...
    axes[0, 1].imshow(gradcam.visualize(image, gradcam_heatmap))
    axes[0, 1].set_title('GradCAM Overlay', fontweight='bold')
    axes[0, 1].axis('off')

    axes[0, 2].imshow(saliency.visualize(image, saliency_map))
    axes[0, 2].set_title('Saliency Overlay', fontweight='bold')
    axes[0, 2].axis('off')

    # Row 2: Heatmaps
    axes[1, 0].imshow(image)
    axes[1, 0].set_title(f'True: {CIFAR10_CLASSES[true_label]}', fontweight='bold')
    axes[1, 0].axis('off')
    ...
    fig.suptitle(f'XAI Demo - Predicted: {CIFAR10_CLASSES[pred_class]} | True: {CIFAR10_CLASSES[true_label]}',
    ...
    save_path = output_dir / 'simple_xai_demo.png'
    ...
    print("\nTo train the model, run: python train_cifar10.py")
